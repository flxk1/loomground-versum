...
nodes/relations. Runtime knowledge produced by an agent — a fact it observed, an inference
it walked — has no source file. This module builds a *valid* dimensioned-subgraph envelope
for such source-less runtime knowledge and writes it through the **same** sink, so the result
is searchable through the existing :func:`versum.store.retrieve.from_dimensioned_store`
read path and is subject to the same erasure / distribution / hierarchy machinery. The sink's
contract and the file-ingest path are untouched; this is purely additive.
...
* **source** — a *synthetic* runtime source shared by everything a given actor asserts::

      source_id      = "runtime:<slug(actor)>"
      content_digest = sha256 of the canonical JSON of the knowledge (triple / path)

  It is not a file digest; it is a stable digest of the asserted knowledge, which is all the
  sink requires (a valid ``sha256:<64hex>`` bound to the envelope).

* **evidence** — one *synthetic* runtime evidence entry (plus any caller-supplied captures)::
      ...
      content_digest = the same knowledge digest as the source

  ``observed_at`` is metadata: it is stamped into the locator and node/relation properties
  but is deliberately **not** part of the idempotency key (see below). Optional ``captures``
  (an LLM answer, a web-search snippet) are attached here as *additional* evidence entries
  rather than as first-class knowledge nodes — see :func:`append_fact`.
...
* **nodes** — ``node_type="entity"``; ``node_id="entity:<slug(term)>:<digest10(term)>"`` (the
  ...
  stable/idempotent across appends). Each node carries a **scalar** coordinate on the declared
  axis: for a fact triple the subject sits at the object value along that axis
  (``dimensions={dimension: object}``) — the fact *places* the subject at the object on the
  chosen dimension. The subject node's ``statement`` property carries the full canonical
  triple text so all three terms are searchable through ``search_similar`` (which derives a
  Doc's text from ``statement``/``bearer``/``action``).

* **relations** — ``relation_type=slug(predicate)`` (the raw predicate is kept in
  ``properties.predicate``); endpoints reference the entity node ids; ``dimension`` is the
  declared axis; ``evidence_ids`` list the runtime evidence (and any captures).
...
:func:`append_fact` / :func:`append_inference`) rather than promoted to a first-class node.
This keeps the knowledge plane (nodes/relations) clean while preserving provenance.
...
def _entity_node(term: str, *, axis: str, coordinate: str, statement: str,
...
def _runtime_source(actor: str, knowledge_digest: str) -> dict:
    return {"source_id": f"runtime:{_slug(actor)}", "content_digest": knowledge_digest}


def _evidence_entries(actor: str, source_id: str, observed_at: str | None,
                      knowledge_digest: str, captures) -> tuple[list[dict], list[str]]:
    """The synthetic runtime evidence entry plus any caller-supplied captures.

    Returns ``(entries, evidence_ids)``; every entry binds to ``source_id`` (the sink demands
    all evidence share the envelope source). ``captures`` is an optional sequence of mappings
    ``{"kind": "llm_answer"|"websearch"|…, "content": str, "locator"?: str}``.
    """
    base_id = "evidence:runtime:" + _short([source_id, knowledge_digest])
...
def append_fact(store_root: str | Path, *, subject: str, predicate: str, object: str,
    ...
    writes it through the sink. See the module docstring for the exact envelope conventions.
    Returns the sink :class:`~versum.ingestion.subgraph.UpsertReceipt` (``status`` is
    ``"inserted"`` the first time, ``"unchanged"`` on an identical re-append).
    ...
    source = _runtime_source(actor, knowledge_digest)
    evidence, evidence_ids = _evidence_entries(
        actor, source["source_id"], observed_at, knowledge_digest, captures)

    subject_node = _entity_node(
        subject, axis=axis, coordinate=object, statement=triple_text,
        bearer=subject, action=object, kind="runtime-fact")
    object_node = _entity_node(
        object, axis=axis, coordinate=object, statement=object, kind="runtime-entity")
...
def append_inference(store_root: str | Path, *, path: Sequence[Mapping[str, Any]],
    ...
    through the sink under the same runtime conventions as :func:`append_fact`. Returns the
    sink :class:`~versum.ingestion.subgraph.UpsertReceipt`.
    """
    axis = _axis(dimension)
    if not isinstance(actor, str) or not actor.strip():
        raise RuntimeCaptureError("actor must be a non-empty string")
    ...
    def _remember(term: str, statement: str) -> str:
        node = _entity_node(term, axis=axis, coordinate=term, statement=statement,
                            bearer=term, kind="runtime-inference")
